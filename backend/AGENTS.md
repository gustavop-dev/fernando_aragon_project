...
> **Fernando Aragón override**: the 3 existing DRF views in `backend/base_feature_app/views/` (`submit_contact_form`, `verify_captcha`, `get_site_key`) are **function-based with `@api_view`**. Pattern: deserialize → service call → respond. Permissions: `AllowAny`. Never convert to CBV/APIView/ViewSets unless explicitly requested.
...
2. **Run only modified test files**: `pytest base_feature_app/tests/path/to/test_file.py -v`. `pytest.ini` sets `DJANGO_SETTINGS_MODULE` and adds `--cov=base_feature_app`.
